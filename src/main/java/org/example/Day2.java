...
public class Day2 {
    ...
    public Day2(String puzzle) {
        String levels = """
                ...
                33 35 36 37 39 41
                """;
        PUZZLE = Objects.requireNonNullElse(puzzle, levels);
    }

    public void solvePartOne() {

        // teile in ein Stream aus Arrays ohne Zeilenumbrüche, filtere leere Strings,
        // wandel jede Zahl in jede, Array zu einer int um
        List<List<Integer>> levels = Arrays.stream(PUZZLE.split("\n"))
                .filter(x -> !x.isEmpty())
                .map(x -> x.split(" "))
        ...
        for (List<Integer> level : levels) {
            ...
            for (int i = 0; i < level.size() - 1; i++) {
                // prüfe ob die Folge steigt oder fällt
                if (i == 0 && level.get(i) > level.get(i + 1)) {
                    increasing = false;
                }
                // wenn die folge nicht steigt obwohl increasing oder andersrum, beende die Berechnung
                if (!((level.get(i) >= level.get(i + 1) && !increasing)
                        || (level.get(i) <= level.get(i + 1) && increasing))) {
                    update = false;
                    break;
                }
                int diff = Math.abs(level.get(i) - level.get(i + 1));
                // prüfe ob jeder Abstand zwischen 1 und 3 liegt
                if (diff > 3 || diff < 1) {
                    update = false;
                    break;
                }
        ...
        System.out.println("The sum of safe levels is: " + res);
    }
    ...
    public void solvePartTwo() {
        ...
        for (List<Integer> level : levels) {
            for (int j = 0; j < level.size(); j++) {
                boolean increasing = true;
                boolean update = true;
                // entferne jeden Eintrag der Liste einmal und prüfe ob sie dann valide ist
                ...
                for (int i = 0; i < tmp.size() - 1; i++) {
                    // prüfe ob die Folge steigt oder fällt
                    if (i == 0 && tmp.get(i) > tmp.get(i + 1)) {
                        increasing = false;
                    }
                    // wenn die folge nicht steigt obwohl increasing oder andersrum, beende die Berechnung
                    if (!((tmp.get(i) >= tmp.get(i + 1) && !increasing)
                            || (tmp.get(i) <= tmp.get(i + 1) && increasing))) {
                        update = false;
                        break;
                    }
                    int diff = Math.abs(tmp.get(i) - tmp.get(i + 1));
                    // prüfe ob jeder Abstand zwischen 1 und 3 liegt
                    if (diff > 3 || diff < 1) {
                        update = false;
                        break;
        ...
        System.out.println("The sum of safe adjusted levels is: " + res);
    }
}
